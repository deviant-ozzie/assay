...
| **Channel D (source SHA)** | Adopter asked for `main` (or any commit that is **not** a release asset) | `statusgen-source` / `desk-tools-source` **full git SHA** | **Yes.** A Go 1.25 toolchain, `go build` of `statusgen` and the desk-tools, and CI that clones that SHA and rebuilds. Copying a release sha256 onto a source pin is a lie. |
...
1. Clone this repository at the SHA the adopter named. Record it as
   `statusgen-source <full SHA> channel-D` and `desk-tools-source <full SHA> channel-D` in the
   **adopter repo's** `.assay-versions` (replace the whole pin lines on upgrade; never silent
   in-place hash edits).
2. `go build` `statusgen` and the desk-tool cmds into `%LOCALAPPDATA%\Assay\bin` (same dest as
   ...
   is expected.
3. Scaffold with `statusgen init --forge <github|gitlab> --root <adopter>`. GitLab CI should
