...
| 9 | check:ci | `cd tools/desk && go test ./cmd/deskclaim/... ./internal/deskkit/... ./internal/loopengine/... -skip '^(TestRegistryCoversCmdBinaries\|TestReStampRecoveryClearsWithoutWeakeningFloor)$' -count=1` | exit 0 — the brief's own (`deskclaim`), touched (`deskkit`, incl. the `TestAcquire*` in row 8) and consumer (`loopengine`) packages; the two `-skip`'d tests are #555's module-wide debt untouched by this brief (`deskinstall` tool-key registration, model-stamp floor recovery). CI still runs the whole suite on the PR. |
...
| 9 | `go test ./cmd/deskclaim/... ./internal/deskkit/... ./internal/loopengine/... -skip '^(TestRegistryCoversCmdBinaries\|TestReStampRecoveryClearsWithoutWeakeningFloor)$' -count=1` | 0 | Re-scoped to the brief's own + consumer packages, excluding #555's two unrelated deskkit reds: `./cmd/deskclaim/...` ok, `./internal/deskkit/...` ok, `./internal/loopengine/...` ok (re-run on current main 2026-09-06). CI runs the whole suite on the PR. |
