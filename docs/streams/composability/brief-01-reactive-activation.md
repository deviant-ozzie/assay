...
sources:
  ...
  - "docs/roster-configuration.md — the trust/extension split of the ASSAY_* keys; trust keys are fail-closed by design"
...
- **create** `tools/desk/internal/deskkit/activation.go` — given the parsed manifests (brief
...
- **edit** `tools/desk/component.yaml` and the role-skill manifests only if the inventory shows
  a verb-to-component mapping the manifests do not yet carry.
- **edit** `docs/roster-configuration.md` — one paragraph: what an invalid extension key now
...
- **Which verb belongs to which component comes from the manifests**, not from a table in
  code. `tools/desk/component.yaml` provides `assay.desk.verbs`; the role skills provide
...
- If a key's classification (trust vs extension) is ambiguous in `docs/roster-configuration.md`,
...
5. Document the behaviour change in `docs/roster-configuration.md`.
