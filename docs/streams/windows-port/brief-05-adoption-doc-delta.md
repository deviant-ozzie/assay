...
| 4b | The doc's own deferral sentence is retired too — grep `adopting-assay.md`'s verbatim wording. The sentence WRAPS across two source lines (`**Windows** is a named` / `fast-follow, not yet in scope`), so the pattern must be a fragment that sits wholly on one line, or a line-oriented grep can never match it: `grep -qiF "fast-follow, not yet in scope" docs/adopting-assay.md; echo $?` | `1` — gone. Non-vacuous by construction: on pre-brief main this exact grep returns `0` (`grep -c` counts 1 hit, the wrapped continuation line in the Prerequisites list). |
...
|---|---------|------|--------|------|--------|
| 1 | `grep -qiE '^## .*Windows' docs/adopting-assay.md` | 0 | `## Windows adopters` section present | 2026-09-06 | opus-4.8[1m] worker (implementer) |
| 2 | `grep -qE -e 'statusgen-windows-amd64[.]exe' -e 'statusgen-windows-arm64[.]exe' docs/adopting-assay.md` | 0 | both `.exe` asset names present (Install path + `.assay-versions` pin block) | 2026-09-06 | opus-4.8[1m] worker |
| 3 | `grep -qiE -e 'sha256' -e 'hash' … && grep -qiE -e 'refus' -e 'verif' …` | 0 | sha256-verify-or-refuse control documented | 2026-09-06 | opus-4.8[1m] worker |
| 4 | `grep -qiF "Windows is a deferred fast-follow" plugins/assay/skills/install/SKILL.md` | 1 | deferral sentence removed (replaced by the real-path pointer) | 2026-09-06 | opus-4.8[1m] worker |
| 4a | `grep -qiF 'windows' plugins/assay/skills/install/SKILL.md` | 0 | Windows still referenced — now as a supported path | 2026-09-06 | opus-4.8[1m] worker |
| 4b | `grep -qiF "fast-follow, not yet in scope" docs/adopting-assay.md` | 1 | doc's Prerequisites deferral retired | 2026-09-06 | opus-4.8[1m] worker |
| 5 | `grep -qiE -e 'CI-proven' -e 'windows CI' -e 'windows-latest' docs/adopting-assay.md && grep -rqE 'runs-on: *windows-latest' .github/workflows/` | **1 — BLOCKED** | Doc's CI claim IS present and windows-latest is named, but the second grep is empty: **windows-port/04 is STAGED (PR #569), its `windows-ci-leg.yml` lives at `ci/staged-workflows/`, NOT `.github/workflows/`** — a workflow-file promotion needs a maintainer's workflow-scoped credential. The doc documents this HONESTLY as "staged, pending promotion — not yet a live check" and does NOT overclaim a green leg. Row goes green once #569 is merged AND promoted into `.github/workflows/`. Never greened from the staged file. | 2026-09-06 | opus-4.8[1m] worker |
| 6 | `grep -qiE -e 'git.?bash' -e 'jq' -e 'SessionStart' -e 'hook' docs/adopting-assay.md && grep -qiF 'documented-workaround' docs/streams/windows-port/portability-audit.md` | 0 | Known-gap references the real triaged SessionStart-hooks `documented-workaround` surface | 2026-09-06 | opus-4.8[1m] worker |
| 7 | `grep -qiF 'native' docs/adopting-assay.md && grep -qiF 'wsl' docs/adopting-assay.md` | 0 | native-Windows claim + WSL-fallback note both present | 2026-09-06 | opus-4.8[1m] worker |
| 8 | `statusgen --root . --consumers windows-port/05` (statusgen built from this repo's own `statusgen/` source — the repo's CI oracle) | 0 | exit 0; the tool renders both prose consumers UNCHECKED = reviewer's-call per brief-rule 9 (a docs delta's fulfilment is a reading judgement, not a mechanical diff match). Presence is ground-truth: `git diff origin/main HEAD --stat` shows `docs/adopting-assay.md` +129 and `plugins/assay/skills/install/SKILL.md` +36/-21 — both fixed-here edits are in the branch diff | 2026-09-06 | opus-4.8[1m] worker |

**Implementer note (honesty ledger).** Rows 1–4b, 6, 7, 8 pass. **Row 5 is BLOCKED, not failed and not greened:** brief 04's Windows CI leg is authored but STAGED in PR #569 — its workflow lives under `ci/staged-workflows/` (filename windows-ci-leg.yml), NOT yet in `.github/workflows/`; an agent credential cannot push a workflow file, so it awaits a maintainer's promotion. The doc reflects the delivered state exactly: it names the staged leg as the pending corroboration and does not claim a live green windows-latest check. The native `windows/arm64` smoke is likewise stated BLOCKED (no arm64 Windows runner), per brief 04. Every other claim maps to a merged artifact: brief 01 release matrix (PR #464), brief 02 portability audit (PR #413, verified), brief 03 install path (PR #513). Lint note: `statusgen --root . --lint` is `0` (PASS) when statusgen is built from this repo's own `statusgen/` source — the repo's CI oracle; the staged-leg filename is referenced un-backticked here so the link-check does not read a not-yet-landed file as a broken link. A stale PATH release binary is not the oracle for this repo (it builds statusgen from source).
