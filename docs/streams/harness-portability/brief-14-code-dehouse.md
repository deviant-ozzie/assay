...
7. **Sweep before every push**, per Verify row 2, and let the public gate speak for itself
   ...
   private review-notes channel and fix exactly what it names. Do not guess from the diff.
...
| 1a | `git ls-files tools/harnessgen tools/harnesslint tools/plugindrift plugins/assay/codex plugins/assay/cursor plugins/assay/.codex-plugin \| wc -l` | `33` — the six added directories contribute exactly 10 + 13 + 8 + 2 + 2 + 1 tracked files. A count, not a presence test: row 1 passes on a directory containing one file, this one does not |
...
| 3 | The `leak-sweep` commit status on this PR's head is `success` — `gh pr view --json statusCheckRollup \| jq -r '.statusCheckRollup[] \| select(.context=="leak-sweep" or .name=="leak-sweep") \| .state // .conclusion'` | `SUCCESS` — the control-based gate, produced elsewhere under a token this session does not hold, agrees with row 2. It posts on its own cadence; a pending status is a wait, and a red one is read from the private review-notes channel, never guessed from the diff |
