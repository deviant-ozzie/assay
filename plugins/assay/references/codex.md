...
| `capability:stop-worker` | V1 `close_agent(id)` / V2 `interrupt_agent(id)` (§3.5/§3.6) — terminate one dispatched child by id. Where `multi_agent` is off there are no children to stop and the desk window runs the fan-out serially, so a per-run stop reaches its single in-flight worker as the cooperative STOP.run.<key> flag alone (Guard refuses that run's next desk verb) — the convenience-degradation floor, stated in-session; the flag layer is never degraded. |
...
| `install` | **runs** — the turnkey install verb; needs `resident-rules-channel` §3.1, `skills-discovery` §3.2 and `install-mechanism` §3.10, all `supported`. Its scaffold write (`statusgen init`, CI + main-guard wiring, re-pin) needs a write-capable sandbox and its pinned-binary acquisition needs network → both are sandbox preconditions (§3.8, blocked under `workspace-write`), stated in-session rather than half-run. It opens draft PRs only and escalates every human-gated step; no dispatch/isolation/gate surface. _(proposed — pending the driver's ruling on the de-house decision issue #626)_ |
...
| `pdfingest` | **runs** — local PDF ingestion, no dispatch/isolation/gate surface; `workspace-write` allows the local reads. Any outbound fetch or external-binary call is a sandbox precondition (§3.8) → the skill states the blocked fetch in-session rather than silently skipping it. _(proposed — pending the driver's ruling on the de-house decision issue #626)_ |
...
| `upgrade-assay` | **runs** — the dry-run-first order is the skill's own invariant and is never degraded; the apply step writes the tree and version pins → needs a write-capable sandbox, and under `workspace-write` it states BLOCKED rather than half-applying (§3.8). No dispatch/isolation/gate surface beyond that. _(proposed — pending the driver's ruling on the de-house decision issue #626)_ |
