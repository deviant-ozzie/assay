...
| `capability:stop-worker` | IDE: stop a background/subagent from the agent list; headless: terminate the `cursor-agent` child process driving the run (§2.5/§2.6). Whether a headless parallel worker is individually addressable for a targeted stop is `[needs: live-install confirmation]` (§2.5) — where it is not, the per-run stop reaches its worker as the cooperative STOP.run.<key> flag alone (Guard refuses that run's next desk verb), the convenience floor stated in-session; the flag layer is never degraded. |
| `capability:durable-monitor` | **No confirmed durable cross-turn wake signal.** The §2.7 completion signals are per-turn, not a re-arming poll, and `cursor-agent` session continuation (`--resume`) has `partial`/unconfirmed durability across process restarts (§2.6) — so there is no verified re-arming monitor that survives a session restart. A durable wake-signal is a **convenience**, not one of the three never-degrade guarantees (isolation / evidence / gates) → the convenience floor applies: the capability **degrades** unless a live install confirms a durable poll (`[needs: live-install confirmation]`, §2.6/§2.7). The skill falls back to the **event-driven + fixed-cadence board sweep** (the same backstop Claude Code pairs with its Monitor — the fixed-cadence sweep is the real liveness safeguard, the durable monitor only ever best-effort) and **states the gap in-session**. The durable liveness home is the always-on observability service, not the harness. |
...
| `ask-decision` | **runs** — interactive decision text; its one write (filing or commenting the decision issue) is a `gh` write → permission-posture precondition (§2.8), else it composes the decision text and states the write is blocked. No dispatch/isolation/gate surface. _(proposed — pending the driver's ruling on the de-house decision issue #626)_ |
...
| `install` | **runs** — the turnkey install verb; needs `resident-rules-channel` §2.1, `skills-discovery` §2.2 and `install-mechanism` §2.10, all `supported`. Its scaffold write and pinned-binary acquisition need a write-capable, network-permitted posture → permission-posture preconditions (§2.8), stated in-session rather than half-run. It opens draft PRs only and escalates every human-gated step; no dispatch/isolation/gate surface. _(proposed — pending the driver's ruling on the de-house decision issue #626)_ |
...
| `pdfingest` | **runs** — local PDF ingestion, no dispatch/isolation/gate surface; the default posture allows local reads. Any outbound fetch or external-binary call is a permission-posture precondition (§2.8) → the skill states the blocked fetch in-session rather than silently skipping it. _(proposed — pending the driver's ruling on the de-house decision issue #626)_ |
...
| `upgrade-assay` | **runs** — the dry-run-first order is the skill's own invariant and is never degraded; the apply step writes the tree and version pins → needs a write-capable posture, and where the posture blocks the write it states BLOCKED rather than half-applying (§2.8). No dispatch/isolation/gate surface beyond that. _(proposed — pending the driver's ruling on the de-house decision issue #626)_ |
